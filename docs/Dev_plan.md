...
The following is a running list of notable elements and components we expect
to need:

* `PluginDirectory`

  Our main `plugin_class` class and hence an MT::Plugin subclass.
...
* `PluginDirectory::Blog`
  ...
  A MT::Blog subclass which represents the Plugin Directory blog

* `PluginDirectory::Client`

  Base class for all PluginDirectory clients
...
* `PluginDirectory::ConfigMgr`

  A subclass of MT::ConfigMgr with additional intelligence and methods
  required to locate, manage and merge not one but *two* YAML config files.

* `PluginDirectory::Entry`

  MT::Entry with extra capabilities and intelligence. This class will play a
  critical role in marshaling all of the metadata and information gathered
  about a PluginDirectory::Item into and out of an entry record and, surely, a
  large number of `MT::Entry::Meta` records which it will define.

* `PluginDirectory::Item`

  An MT::Object subclass whose records represent plugins submitted for
  inclusion in the Plugin Directory

* `PluginDirectory::Item::ConfigYAML`

  A class which represents a plugin's `config.yaml` file and contains methods
  needed by both `PluginDirectory::Worker` subclasses to validate the file's
  contents and ensure that it meets our requirements.

* `PluginDirectory::Item::PluginYAML`

  A class which represents a plugin's `plugin.yaml` file and contains methods
  needed by both `PluginDirectory::Worker` subclasses to validate the file's
  contents and ensure that it meets our requirements.
